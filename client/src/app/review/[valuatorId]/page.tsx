  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="flex justify-between z-50 p-5 fixed navbar backdrop-filter backdrop-blur-lg bg-opacity-30  bg-base-100">
        <select className="select select-bordered w-full max-w-xs" value={selectedValuation} onChange={(e) => setSelectedValuation(e.target.value)}>
          {valuations ? valuations?.map((valuation: any, index: number) => {
            return <option key={index} value={index}>{valuation?.data?.student_name}</option>
          }) : ""}
        </select>
            ...
            return <div className="flex my-3 max-w-7xl">
              <div className="collapse shadow-xl border-2">
                ...
                <div className="flex items-center justify-between collapse-title">
                  <p className="text-md font-semibold">{item?.question_no}. {item?.question}</p>
                  <div className="flex items-center">
                    <p className="flex items-center font-semibold"><AiOutlineTrophy className="mr-2" />{item?.score[0]} / {item?.score[1]}</p>
                    <div className="flex flex-col ml-10">
                      <span className="text-sm text-gray-500">Confidence:</span>
                      <progress className={"progress w-20 " + (item?.confidence === 1 ? "progress-success" : item?.confidence === 0 ? "progress-error" : "progress-warning")} value={item?.confidence === 1 ? "100" : item?.confidence === 0 ? "0" : "50"} max="100"></progress>
